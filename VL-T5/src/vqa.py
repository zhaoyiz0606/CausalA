...
from utils import load_state_dict, LossMeter, set_global_logging_level, logging_table
from metaloss import MELTR
import dist_utils
import wandb
...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        self.start_epoch = 0
        if args.resume:
            if os.path.exists(os.path.join(args.output, 'checkpoint.pth')):
                args.resume = os.path.join(args.output, 'checkpoint.pth')
        ...
        if args.load is not None and args.resume is None:
        ...
        #####
        self.auxiliary_combine_net = MELTR(t_dim=args.taskNum, f_dim=args.transformer_dim[0], i_dim=1,
                                           h1_dim=args.transformer_dim[1], h2_dim=args.transformer_dim[2], o_dim=1).to(args.gpu)
        auxiliary_param = list(self.auxiliary_combine_net.parameters())

        self.meta_opt = torch.optim.Adam(auxiliary_param, lr=args.lr_vnet, weight_decay=args.decay_vnet)
        ...
        if args.multiGPU:
            if args.distributed:
                ...
                self.auxiliary_combine_net = DDP(self.auxiliary_combine_net, device_ids=[args.gpu],
                                                 find_unused_parameters=True
                                                 )
        self.val_score_list = []
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            for step_i, batch in enumerate(self.train_loader):
                if self.args.fp16 and _use_native_amp:
                    ...
                    if self.args.distributed:
                        results = self.model.module.train_step(batch)
                        losses = [results[i] for i in results.keys()]
                        if epoch > 1:
                            loss = self.auxiliary_combine_net(torch.stack(losses).unsqueeze(1))
                        else:
                            loss = sum(losses)

                        # losses = [results[i] for i in results.keys()]
                        # temp = torch.stack(losses)
                        # tmp = temp.unsqueeze(1)
                        # loss = self.auxiliary_combine_net(torch.stack(losses).unsqueeze(1))
                    else:
                        results = self.model.train_step(batch)
                ...
                if self.args.clip_grad_norm > 0:
                    if self.args.fp16 and _use_native_amp:   #False
                        self.scaler.unscale_(self.optim)
                        torch.nn.utils.clip_grad_norm_(
                            self.model.parameters(), self.args.clip_grad_norm)
                    elif self.args.fp16 and _use_apex:  #False
                        ...
                        #####
                        torch.nn.utils.clip_grad_norm_(
                            self.auxiliary_combine_net.parameters(), self.args.clip_grad_norm)
                ...
                for param in self.model.parameters():
                    param.grad = None
                #####
                for param in self.auxiliary_combine_net.parameters():
                    param.grad = None
        ...
        # Test val Set, not best test set
        best_path = os.path.join(self.args.output, 'checkpoint_BEST')
        self.load(best_path)
        ...
        if self.verbose:
            ...
            results_path = os.path.join(self.args.output, 'quesid2ans_results.json')
            evaluator.dump_result(quesid2ans, results_path)
        ...
        print(logging_table(list(range(len(self.val_score_list))), self.val_score_list, self.test_score_list))

        if self.args.distributed:
            dist.barrier()
            exit()

    def print_log(self, epoch, dataloader, epoch_results=None):
        score_dict = self.evaluate(dataloader)
        #####
        # name = str(datetime.datetime.now().strftime("%Y-%m-%d"))
        self.save(epoch=epoch)
        #####
        valid_score = score_dict['topk_score'] * 100.
        self.val_score_list.append(float("%.2f" % valid_score))
        valid_score_raw = score_dict['overall']
        if valid_score_raw > self.best_valid or epoch == 0:
            self.best_valid = valid_score_raw
    ...
    def test(self, args, test_loader):
        ...
        if not args.test_only:
            best_path = os.path.join(self.args.output, 'checkpoint_BEST')
            if os.path.exists(best_path + '.pth'):
                self.load(best_path)
                print("Load best model from {}".format(best_path + '.pth'))

        quesid2ans = self.predict(test_loader)
...
if __name__ == "__main__":
    ...
    args.taskNum = len(LOSSES_NAME) + 1
