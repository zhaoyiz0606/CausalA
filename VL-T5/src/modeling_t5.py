...
# from utils import *
from modeling_adapter import CausalAdapter
...
class VLT5(T5ForConditionalGeneration):
    ...
    def __init__(self, config, freeze_joint_encoder=True):
        ...
        ###### add casual adapter
        self.causal_adapter = CausalAdapter(encoder_config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            # encoder_outputs为(bs,56,768),其中20维文本特征,36维视觉特征
        ...
        #####Add Causal Adapter
        # hidden_states_causal = self.causal_adapter(hidden_states[:,:20,:], attention_mask, hidden_states[:,20:,:])
        hidden_states_causal = self.causal_adapter(hidden_states, encoder_attention_mask)
        # Decode
        merge_hidden_states = self.causal_gate.tanh() * hidden_states_causal + hidden_states
        # merge_hidden_states = hidden_states_causal
        decoder_outputs = self.decoder(
            ...
            # encoder_hidden_states=hidden_states_causal,
            encoder_hidden_states=merge_hidden_states,
