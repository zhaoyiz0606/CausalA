...
dataset_dir = workspace_dir.joinpath('/data/***/VQACL/').resolve()
...
class VQAFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #####lm loss
        if 'lm_loss' in self.args.LOSSES_NAME:
            prefix = "span prediction:"
            # sent = datum['sent']
            sent = datum['caption']
            lm_source_text, lm_target_text = preprocess.corrupt_spans(
                sent, mask_ratio=self.args.word_mask_rate, prefix=prefix)

            # if self.args.oscar_tags:
            #     input_tokens = [lm_source_text]
            ...
            #     lm_source_text = ' '.join(input_tokens)

            lm_input_ids = self.tokenizer.encode(
                lm_source_text, padding=True, truncation=True, max_length=self.args.max_text_length
            )
            lm_target_ids = self.tokenizer.encode(
                lm_target_text, padding=True, truncation=True, max_length=self.args.gen_max_length
            )
            out_dict['lm_input_ids'] = torch.LongTensor(lm_input_ids)
            out_dict['lm_target_ids'] = torch.LongTensor(lm_target_ids)

        #####itm_loss
        if 'itm_loss' in self.args.LOSSES_NAME:
            is_matched = 1
            # sent = datum['sent']
            sent = datum['caption']
            if random.random() < 0.5:
                ...
                other_datum = self.data[rand_idx]
                while other_datum['img_id'] == img_id:
                    rand_idx = random.randint(0, len(self.data) - 1)
                    other_datum = self.data[rand_idx]

                # sent = other_datum['sent']
                sent = other_datum['caption']

            prefix = "image text match:"
            itm_source_text = f"{prefix} {sent}"
            # if self.args.oscar_tags:
            ...
            if is_matched:
                itm_target_text = 'true'
            else:
                itm_target_text = 'false'

            itm_input_ids = self.tokenizer.encode(
                itm_source_text, padding=True, truncation=True, max_length=self.args.max_text_length
            )
            itm_target_ids = self.tokenizer.encode(
                itm_target_text, padding=True, truncation=True, max_length=self.args.gen_max_length
            )
            out_dict['itm_input_ids'] = torch.LongTensor(itm_input_ids)
            out_dict['itm_target_ids'] = torch.LongTensor(itm_target_ids)
    ...
    #####
    def str2txt(self, s):
        txt = self.tokenizer.encode(s, padding='max_length', max_length=self.args['size_txt'], truncation=True)
    ...
    def collate_fn(self, batch):
        ...
        #####
        lm_S_W_L = max(len(entry['lm_input_ids']) for entry in batch)
        # lm_S_W_L = 20
        lm_T_W_L = max(len(entry['lm_target_ids']) for entry in batch)
        lm_input_ids = torch.ones(B, lm_S_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
        lm_target_ids = torch.ones(B, lm_T_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
        itm_S_W_L = max(len(entry['itm_input_ids']) for entry in batch)
        itm_T_W_L = max(len(entry['itm_target_ids']) for entry in batch)
        itm_input_ids = torch.ones(B, itm_S_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
        itm_target_ids = torch.ones(B, itm_T_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
        ...
        for i, entry in enumerate(batch):
            ...
            lm_input_ids[i, :len(entry['lm_input_ids'])] = entry['lm_input_ids']
            itm_input_ids[i, :len(entry['itm_input_ids'])] = entry['itm_input_ids']
            ...
            if 'target_ids' in entry:
                ...
                lm_target_ids[i, :len(entry['lm_target_ids'])] = entry['lm_target_ids']
                itm_target_ids[i, :len(entry['itm_target_ids'])] = entry['itm_target_ids']
        ...
        batch_entry['lm_input_ids'] = lm_input_ids
        batch_entry['itm_input_ids'] = itm_input_ids
        if 'target_ids' in batch[0]:
            word_mask = target_ids != self.tokenizer.pad_token_id
            target_ids[~word_mask] = -100
            batch_entry['target_ids'] = target_ids

            word_mask = lm_target_ids != self.tokenizer.pad_token_id
            lm_target_ids[~word_mask] = -100
            batch_entry['lm_target_ids'] = lm_target_ids

            word_mask = itm_target_ids != self.tokenizer.pad_token_id
            itm_target_ids[~word_mask] = -100
            batch_entry['itm_target_ids'] = itm_target_ids
        if 'target' in batch[0]:
